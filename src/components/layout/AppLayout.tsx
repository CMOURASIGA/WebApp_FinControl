...
import { Bell, Briefcase, CalendarCheck, ChevronDown, CircleDollarSign, LayoutDashboard, LogOut, Menu, ReceiptText, SlidersHorizontal, TrendingUp, Users, X } from 'lucide-react';
    ...
    <div className="min-h-screen bg-[var(--bg-page)] md:flex">
      ...
      <aside className={`sidebar-shell fixed inset-y-0 left-0 z-40 flex w-[256px] flex-col overflow-hidden transition-transform md:sticky md:top-0 md:h-screen md:translate-x-0 ${menuAberto ? 'translate-x-0' : '-translate-x-full'}`}>
        <div className="relative border-b border-white/15 px-4 pb-6 pt-4">
          <div className="flex h-[144px] w-full items-center justify-center overflow-hidden rounded-xl bg-white px-0.5 py-1 shadow-sm">
          ...
          <div className="mt-6"><p className="brand-highlight-text text-[11px] font-black uppercase tracking-[0.22em]">{brand.product_name}</p><p className="mt-1.5 text-sm font-semibold leading-5 text-white">{brand.product_subtitle}</p></div>
        </div>
        <nav className="flex-1 overflow-y-auto px-3 py-4">
          {sections.map((section) => <div key={section} className="mb-5"><p className="mb-2 px-3 text-[10px] font-black uppercase tracking-[0.16em] text-cyan-100/65">{section}</p><div className="space-y-1">{itensVisiveis.filter((item) => item.section === section).map(({ to, label, icon: Icon, end }) => <NavLink key={to} to={to} end={end} onClick={() => setMenuAberto(false)} className={({ isActive }) => `flex items-center gap-3 rounded-xl px-3 py-2.5 text-sm font-medium transition ${isActive ? 'brand-highlight-bg text-slate-950 shadow-sm' : 'text-slate-100 hover:bg-white/10 hover:text-white'}`}><Icon className="h-4 w-4 shrink-0" /><span>{label}</span></NavLink>)}</div></div>)}
        </nav>
      </aside>
      <div className="flex min-h-screen min-w-0 flex-1 flex-col">
        <header className="sticky top-0 z-20 flex min-h-16 items-center justify-between gap-4 border-b border-slate-200 bg-white/95 px-4 py-2 backdrop-blur md:px-6">
          <div className="flex min-w-0 items-center gap-3"><button className="rounded-xl border border-slate-200 p-2 text-slate-600 md:hidden" onClick={() => setMenuAberto(true)} aria-label="Abrir menu"><Menu className="h-5 w-5" /></button><div className="min-w-0"><p className="brand-highlight-text truncate text-[10px] font-black uppercase tracking-[0.2em]">Workspace financeiro</p><h1 className="truncate text-base font-semibold text-slate-900">{titulo}</h1></div></div>
          <div className="flex items-center gap-2 md:gap-3"><span className="hidden items-center gap-2 rounded-full border border-emerald-200 bg-emerald-50 px-3 py-1.5 text-[12px] font-semibold text-emerald-700 sm:inline-flex"><span className="h-2 w-2 rounded-full bg-emerald-500" />Sistema online</span><button type="button" className="hidden rounded-full p-2 text-slate-500 hover:bg-slate-100 sm:inline-flex" aria-label="Notificações"><Bell className="h-4 w-4" /></button>
            <div ref={userMenuRef} className="relative"><button type="button" onClick={() => setUserMenuAberto((aberto) => !aberto)} className="flex items-center gap-2 rounded-xl px-1.5 py-1 transition hover:bg-slate-100 sm:px-2" aria-expanded={userMenuAberto} aria-haspopup="menu"><span className="brand-highlight-bg flex h-9 w-9 shrink-0 items-center justify-center rounded-full text-xs font-bold text-slate-950">{iniciais || 'US'}</span><span className="hidden min-w-0 text-left md:block"><span className="block max-w-40 truncate text-sm font-semibold text-slate-900">{nomeUsuario}</span><span className="block text-[10px] uppercase tracking-wide text-slate-500">{papelUsuario}</span></span><ChevronDown className="hidden h-4 w-4 text-slate-400 md:block" /></button>
              {userMenuAberto && <div role="menu" className="absolute right-0 top-[calc(100%+10px)] w-64 overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-xl"><div className="border-b border-slate-100 px-4 py-4"><p className="truncate text-sm font-semibold text-slate-900">{nomeUsuario}</p><p className="mt-1 text-[11px] uppercase tracking-wide text-slate-500">{papelUsuario}</p></div><div className="p-2"><button type="button" onClick={() => signOut()} className="flex w-full items-center gap-3 rounded-xl px-3 py-2 text-left text-sm font-medium text-red-600 hover:bg-red-50"><LogOut className="h-4 w-4" />Sair</button></div></div>}
            </div>
          </div>
        </header>
