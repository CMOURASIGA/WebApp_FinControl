...
import { COMPANY_LOGO_URL, COMPANY_NAME, PRODUCT_NAME, PRODUCT_SUBTITLE } from '../../lib/brand';
  ...
  return (
    <div className="min-h-screen bg-[var(--bg-page)] md:flex">
      ...
      <aside className={`sidebar-shell fixed inset-y-0 left-0 z-40 flex w-[270px] flex-col overflow-hidden transition-transform md:sticky md:top-0 md:h-screen md:translate-x-0 ${menuAberto ? 'translate-x-0' : '-translate-x-full'}`}>
        <div className="sidebar-brand-panel relative"><img src={COMPANY_LOGO_URL} alt={COMPANY_NAME} className="h-20 w-full object-contain" /><button className="absolute right-3 top-3 rounded-full p-2 text-slate-500 md:hidden" onClick={() => setMenuAberto(false)}><X className="h-5 w-5" /></button></div>
        <div className="border-b border-white/15 px-5 py-4"><p className="text-[10px] font-black uppercase tracking-[0.24em] text-cyan-300">{PRODUCT_NAME}</p><p className="mt-1 text-sm font-semibold text-white">{PRODUCT_SUBTITLE}</p><p className="mt-1 text-[10px] text-cyan-100">Uma plataforma {COMPANY_NAME}</p></div>
        <nav className="flex-1 overflow-y-auto px-3 py-4">
          {sections.map((section) => <div key={section} className="mb-5"><p className="mb-2 px-3 text-[10px] font-black uppercase tracking-[0.16em] text-cyan-200/80">{section}</p><div className="space-y-1">{NAV_ITEMS.filter((item) => item.section === section).map(({ to, label, icon: Icon, end }) => <NavLink key={to} to={to} end={end} onClick={() => setMenuAberto(false)} className={({ isActive }) => `flex items-center gap-3 rounded-xl px-3 py-2.5 text-sm font-medium transition ${isActive ? 'bg-cyan-400 text-slate-950 shadow-sm' : 'text-slate-100 hover:bg-white/10 hover:text-white'}`}><Icon className="h-4 w-4" />{label}</NavLink>)}</div></div>)}
        </nav>
        <div className="border-t border-white/15 p-4"><div className="flex items-center gap-3"><span className="flex h-9 w-9 items-center justify-center rounded-full bg-cyan-400 text-xs font-bold text-slate-950">{iniciais}</span><div className="min-w-0 flex-1"><p className="truncate text-sm font-medium text-white">{profile?.nome ?? '...'}</p><p className="text-[10px] uppercase tracking-wide text-cyan-200">{profile?.papel ?? 'usuário'}</p></div><button onClick={() => signOut()} className="rounded-lg p-2 text-cyan-100 hover:bg-white/10 hover:text-white" title="Sair"><LogOut className="h-4 w-4" /></button></div></div>
      </aside>
      <div className="flex min-h-screen min-w-0 flex-1 flex-col">
        <header className="sticky top-0 z-20 flex h-16 items-center justify-between border-b border-slate-200 bg-white/90 px-4 backdrop-blur md:px-6"><div className="flex items-center gap-3"><button className="rounded-xl border border-slate-200 p-2 text-slate-600 md:hidden" onClick={() => setMenuAberto(true)}><Menu className="h-5 w-5" /></button><div><p className="text-[10px] font-black uppercase tracking-[0.2em] text-cyan-600">Workspace financeiro</p><h1 className="text-base font-semibold text-slate-900">{titulo}</h1></div></div><span className="hidden items-center gap-2 rounded-full border border-emerald-200 bg-emerald-50 px-3 py-1 text-[11px] font-medium text-emerald-700 sm:inline-flex"><span className="h-2 w-2 rounded-full bg-emerald-500" />Operação online</span></header>
