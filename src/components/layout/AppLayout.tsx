...
export const AppLayout: React.FC = () => {
  ...
  return (
    <div className="min-h-screen bg-[var(--bg-page)] md:flex">
      ...
      <aside
        className={`sidebar-shell fixed inset-y-0 left-0 z-40 flex w-[256px] flex-col overflow-hidden transition-transform md:sticky md:top-0 md:h-screen md:translate-x-0 ${
      ...
      >
        <div className="relative border-b border-white/15 px-5 pb-5 pt-5">
          ...
          <button
            className="absolute right-3 top-3 rounded-full p-2 text-white/80 hover:bg-white/10 md:hidden"
            onClick={() => setMenuAberto(false)}
            aria-label="Fechar menu"
          >
            <X className="h-5 w-5" />
          ...
          <div className="mt-5">
            <div className="flex items-center gap-2">
              <p className="brand-highlight-text text-[11px] font-black uppercase tracking-[0.22em]">
                {brand.product_name}
              </p>
        ...
        <nav className="flex-1 overflow-y-auto px-3 py-4">
          {sections.map((section) => (
            <div key={section} className="mb-5">
              <p className="mb-2 px-3 text-[10px] font-black uppercase tracking-[0.16em] text-cyan-100/65">
                {section}
              </p>
                  ...
                  <NavLink
                    ...
                    className={({ isActive }) =>
                      `flex items-center gap-3 rounded-xl px-3 py-2.5 text-sm font-medium transition ${
                        isActive
                          ? 'brand-highlight-bg text-slate-950 shadow-sm'
        ...
        <header className="sticky top-0 z-20 flex min-h-16 items-center justify-between gap-4 border-b border-slate-200 bg-white/95 px-4 py-2 backdrop-blur md:px-6">
          <div className="flex min-w-0 items-center gap-3">
            ...
            <div className="min-w-0">
              <p className="brand-highlight-text truncate text-[10px] font-black uppercase tracking-[0.2em]">
          ...
          <div className="flex items-center gap-2 md:gap-3">
            <button
              ...
              className="hidden items-center gap-2 rounded-full border border-amber-200 bg-amber-50 px-3 py-1.5 text-[11px] font-semibold text-amber-800 hover:bg-amber-100 lg:inline-flex"
            >
            ...
            <div ref={userMenuRef} className="relative">
              <button
                ...
                className="flex items-center gap-2 rounded-xl px-1.5 py-1 transition hover:bg-slate-100 sm:px-2"
                aria-expanded={userMenuAberto}
                aria-haspopup="menu"
              >
                <span className="brand-highlight-bg flex h-9 w-9 shrink-0 items-center justify-center rounded-full text-xs font-bold text-slate-950">
                  {iniciais || 'US'}
                </span>
                <span className="hidden min-w-0 text-left md:block">
                  <span className="block max-w-40 truncate text-sm font-semibold text-slate-900">{nomeUsuario}</span>
                  <span className="block text-[10px] uppercase tracking-wide text-slate-500">{papelUsuario}</span>
                </span>
              ...
              {userMenuAberto && (
                <div
                  role="menu"
                  className="absolute right-0 top-[calc(100%+10px)] w-64 overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-xl"
                >
                  <div className="border-b border-slate-100 px-4 py-4">
                    <p className="truncate text-sm font-semibold text-slate-900">{nomeUsuario}</p>
                    <p className="mt-1 text-[11px] uppercase tracking-wide text-slate-500">{papelUsuario}</p>
                  </div>
                  <div className="p-2">
                    <button
                      type="button"
                      onClick={() => setConfirmarReset(true)}
                      className="flex w-full items-center gap-3 rounded-xl px-3 py-2 text-left text-sm text-slate-700 hover:bg-slate-50 lg:hidden"
                    >
      ...
      <Modal
        ...
        descricao="Todos os testes feitos neste navegador serão apagados e os dados fictícios iniciais voltarão."
        onClose={() => setConfirmarReset(false)}
      >
        <div className="flex justify-end gap-2">
          <Button variant="secondary" onClick={() => setConfirmarReset(false)}>
            Cancelar
          </Button>
