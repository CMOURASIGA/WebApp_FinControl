  ...
  return (
    <div className="min-h-screen bg-[var(--bg-page)] md:flex">
      ...
      <aside className={`sidebar-shell fixed inset-y-0 left-0 z-40 flex w-[270px] flex-col overflow-hidden transition-transform md:sticky md:top-0 md:h-screen md:translate-x-0 ${menuAberto ? 'translate-x-0' : '-translate-x-full'}`}>
        <div className="sidebar-brand-panel relative"><img src={brand.logo_url} alt={brand.company_name} className="sidebar-brand-logo" /><button className="absolute right-3 top-3 rounded-full p-2 text-slate-500 md:hidden" onClick={() => setMenuAberto(false)}><X className="h-5 w-5" /></button></div>
