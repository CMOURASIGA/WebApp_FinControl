...
function seed(): Store {
  ...
  return {
    ...
    investimentos:[{...base,id:'demo-investimento-1',investidor_tipo:'socio',socio_id:socio,projeto_id:projeto,valor:1500,data:day(1),tipo:'aporte',descricao:'Capital inicial do projeto',retorno_esperado:3000,prazo_esperado_meses:3,roi_meta_percentual:50,considerado_no_resultado:false,data_encerramento:null,status:'ativo'}],assinaturas:[],socio_lancamentos:[],reserva_empresa_lancamentos:[],fechamentos_mensais:[],receita_historico:[],financeiro_historico:[],investimento_historico:[],
    white_label_settings:[{...DEFAULT_BRAND,id:true,updated_at:now()}]
  };
}
function read():Store { const raw=localStorage.getItem(STORE_KEY); if(raw) try{const s=JSON.parse(raw) as Store;s.investimento_historico??=[];s.investimentos=(s.investimentos??[]).map(r=>({status:'ativo',updated_at:r.updated_at??now(),...r}));return s}catch{} const s=seed(); write(s); return s; }
...
class Query {
  ...
  private async run(){const s=read(),rows=s[this.table]??[],match=(r:Row)=>this.filters.every(f=>f(r));let out:Row[];
    if(this.op==='insert'){out=(Array.isArray(this.payload)?this.payload:[this.payload]).map((r:Row)=>({id:r.id??id(this.table),created_at:r.created_at??now(),updated_at:r.updated_at??now(),...(this.table==='investimentos'?{status:'ativo',data_encerramento:null}:{}),...r}));s[this.table]=[...rows,...out];write(s);}
...
async function rpc(name:string,p:Row){const s=read(),find=(t:string,i:string)=>(s[t]??[]).find(r=>r.id===i);let returned:any=null;
  ...
  else if(name==='editar_investimento'){const r=find('investimentos',p.p_id);if(r){const b={...r};Object.assign(r,{investidor_tipo:p.p_investidor_tipo,socio_id:p.p_investidor_tipo==='socio'?p.p_socio_id:null,projeto_id:p.p_projeto_id,valor:p.p_valor,data:p.p_data,tipo:p.p_tipo,descricao:p.p_descricao||null,retorno_esperado:p.p_retorno_esperado,prazo_esperado_meses:p.p_prazo_esperado_meses,roi_meta_percentual:p.p_roi_meta_percentual,considerado_no_resultado:p.p_considerado_no_resultado,updated_at:now()});s.investimento_historico.push({id:id('hist-investimento'),investimento_id:r.id,acao:'edicao',dados_anteriores:b,dados_novos:{...r},motivo:p.p_motivo,executado_por:USER_ID,executado_em:now()});returned={...r};}}
  else if(name==='alterar_status_investimento'){const r=find('investimentos',p.p_id);if(r){const b={...r},cancelar=p.p_acao==='cancelar';r.status=cancelar?'cancelado':'ativo';r.updated_at=now();s.investimento_historico.push({id:id('hist-investimento'),investimento_id:r.id,acao:cancelar?'cancelamento':'reativacao',dados_anteriores:b,dados_novos:{...r},motivo:p.p_motivo,executado_por:USER_ID,executado_em:now()});returned={...r};}}
